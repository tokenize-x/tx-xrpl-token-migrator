...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::Withdraw {} => withdraw(env, info, deps),
    }
}
...
pub fn threshold_bank_send(
    ...
    // The evidence ID is: `id-denom-amount-recipient` to cover the case when some of the
    ...
    let evidence_id = format!("{id}-{amount}-{recipient}").to_lowercase();
    ...
    if tx.evidence_providers.len() as u32 == THRESHOLD.load(deps.storage)? {
        ...
        let res = Response::new()
            .add_attribute("method", "threshold_bank_send")
...
pub fn withdraw(env: Env, info: MessageInfo, deps: DepsMut) -> Result<Response, ContractError> {
...
mod tests {
    ...
    fn test_threshold_bank_send() {
        ...
        let evidence_id = format!("{id}-{amount}-{recipient}").to_lowercase();
        let malicious_evidence_id = format!("{id}-{malicious_amount}-{recipient}").to_lowercase();
    ...
    fn test_withdraw() {
        ...
        // execute from none owner
        let info = mock_info(TEST_TRUSTED_ADDRESS1, &[]);
        let res = withdraw(env.clone(), info, deps.as_mut());
